...
import { Check, Circle, CircleDot, ArrowLeft, Truck, Shield, CreditCard, BookOpen } from "lucide-react";
    ...
    <div className="flex items-center justify-center gap-8 mb-12">
      {["details", "payment", "confirm"].map((s, i) => (
        <div key={s} className="flex items-center gap-3">
          ...
          {i < 2 && (
            <div className={`w-12 h-0.5 ml-3 transition-colors duration-300 ${
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-[#2D4A3C] mb-4">
              চেকআউট
            </h1>
          </div>
              ...
              {step === "details" && (
                <div className="space-y-6">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-2xl font-bold text-[#2D4A3C]">ব্যক্তিগত তথ্য</h2>
                  </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label htmlFor="deliveryAddress" className="text-sm font-medium text-[#2D4A3C]">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-8 bg-[#819A91] rounded-full"></div>
                      <h2 className="text-2xl font-bold text-[#2D4A3C]">পেমেন্ট পদ্ধতি</h2>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  <div className="grid gap-4">
                    {[
                      { id: "bkash", name: "বিকাশ", color: "bg-gradient-to-r from-pink-500 to-red-500" },
                      { id: "nagad", name: "নগদ", color: "bg-gradient-to-r from-emerald-500 to-green-500" },
                      { id: "rocket", name: "রকেট", color: "bg-gradient-to-r from-purple-500 to-indigo-500" },
                      { id: "CashOnDelivery", name: "ক্যাশ অন ডেলিভারি", color: "bg-gradient-to-r from-[#A7C1A8] to-[#819A91]" }
                    ].map((method) => (
                      <div
                        key={method.id}
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-lg ${method.color} flex items-center justify-center shadow-md`}>
                              <Image
                            ...
                            <div>
                              <span className="font-semibold text-[#2D4A3C]">{method.name}</span>
                              {method.id === "CashOnDelivery" && (
                                <p className="text-sm text-[#2D4A3C]/70 mt-1">ডেলিভারির সময় পেমেন্ট করুন</p>
                              )}
                            </div>
                          </div>
                  ...
                  {paymentMethod && paymentMethod !== "CashOnDelivery" && (
                    <div className="bg-[#EEEFE0] rounded-xl p-6 mt-6 border border-[#D1D8BE]">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h3 className="font-semibold text-[#2D4A3C]">পেমেন্ট নির্দেশনা</h3>
                      </div>
                      <p className="text-sm text-[#2D4A3C] mb-4">
                        পেমেন্ট করুন এই নাম্বারে: <strong className="text-[#2D4A3C]">০১৭XXXXXXXX</strong>
                      </p>
                      <LabeledInput
                        id="transactionId"
              ...
              {step === "confirm" && placedOrder && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-8 bg-[#819A91] rounded-full"></div>
                      <h2 className="text-2xl font-bold text-[#2D4A3C]">অর্ডার নিশ্চিতকরণ</h2>
                    </div>
                    <Button
                      variant="ghost"
                      onClick={() => setStep("payment")}
                  ...
                  <div className="bg-[#A7C1A8]/20 border border-[#A7C1A8] rounded-xl p-6 mb-6">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-semibold text-[#2D4A3C]">অর্ডার সফলভাবে তৈরি হয়েছে!</h3>
                    </div>
                    <p className="text-[#2D4A3C]">
                      Invoice ID: <strong className="text-[#2D4A3C]">{invoiceId}</strong>
                    </p>
                  </div>

                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <h4 className="font-semibold text-[#2D4A3C]">গ্রাহক তথ্য</h4>
                      <div className="space-y-2 text-sm">
                        <p><span className="text-[#2D4A3C]/80">নাম:</span> <span className="text-[#2D4A3C]">{placedOrder.customer.name}</span></p>
                        <p><span className="text-[#2D4A3C]/80">মোবাইল:</span> <span className="text-[#2D4A3C]">{placedOrder.customer.mobile}</span></p>
                        <p><span className="text-[#2D4A3C]/80">ইমেইল:</span> <span className="text-[#2D4A3C]">{placedOrder.customer.email || "N/A"}</span></p>
                        <p><span className="text-[#2D4A3C]/80">ঠিকানা:</span> <span className="text-[#2D4A3C]">{placedOrder.customer.address}</span></p>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <h4 className="font-semibold text-[#2D4A3C]">অর্ডার বিবরণ</h4>
                      <div className="space-y-2 text-sm">
                        <p><span className="text-[#2D4A3C]/80">তারিখ:</span> <span className="text-[#2D4A3C]">{new Date(placedOrder.createdAt).toLocaleDateString('bn-BD')}</span></p>
                        <p><span className="text-[#2D4A3C]/80">সময়:</span> <span className="text-[#2D4A3C]">{new Date(placedOrder.createdAt).toLocaleTimeString('bn-BD')}</span></p>
                        <p><span className="text-[#2D4A3C]/80">পেমেন্ট:</span> <span className="text-[#2D4A3C]">{placedOrder.paymentMethod}</span></p>
                        {placedOrder.transactionId && (
                          <p><span className="text-[#2D4A3C]/80">ট্রান্স্যাকশন:</span> <span className="text-[#2D4A3C]">{placedOrder.transactionId}</span></p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg border border-[#D1D8BE] p-6 sticky top-6">
              ...
              <div className="space-y-4 mb-6 max-h-96 overflow-y-auto">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-3 rounded-lg bg-[#EEEFE0] border border-[#D1D8BE]">
              ...
              <div className="space-y-3 border-t border-[#D1D8BE] pt-4">
                ...
                <div className="flex justify-between font-bold text-lg text-[#2D4A3C] border-t border-[#D1D8BE] pt-3">
                  <span>মোট</span>
                  <span className="text-[#2D4A3C] font-bold">৳{total.toFixed(2)}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center space-y-6 shadow-2xl border border-[#D1D8BE]">
            ...
            <p className="text-[#2D4A3C] leading-relaxed">
              আপনার অর্ডার সফলভাবে গৃহীত হয়েছে। অর্ডার ট্র্যাক করতে নিচের বাটনে ক্লিক করুন।
            </p>
            <div className="space-y-3">
              ...
              <Link href="/books">
                <Button variant="outline" className="w-full border-[#D1D8BE] text-[#2D4A3C] hover:bg-[#EEEFE0] rounded-xl">
                  আরও বই দেখুন
                </Button>
              </Link>
            </div>
